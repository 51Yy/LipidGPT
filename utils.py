...
###Title: SMILES-enumeration
# Repository: https://github.com/EBjerrum/SMILES-enumeration
...
class SmilesIterator(Iterator):
    ...
    def next(self):
        ...
        batch_x = np.zeros(tuple([current_batch_size] + [self.smiles_data_generator.pad,
                           self.smiles_data_generator._charlen]), dtype=self.dtype)
...
def filter_valid_smiles(smiles_list, vocab, regex):
    """
    ...
    for smiles in smiles_list:
        ...
        if all(token in vocab for token in tokens):
            ...
            removed_count += 1
    return filtered_smiles, removed_count
