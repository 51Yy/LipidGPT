...
from utils import set_seed, filter_valid_smiles
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='SMILES Generator Fine-tuning Script')

    parser.add_argument('--debug', action='store_true', default=False, help='debug mode')
    parser.add_argument('--mpath', type=str, default=None, help="pre-trained model path")
    parser.add_argument('--pre_data', type=str, default=None, help='pre-training data name')
    parser.add_argument('--data_name', type=str, default='PosILs_can', help="dataset name for fine-tuning")
    ...
    parser.add_argument('--max_epochs', type=int, default=10, help="maximum number of training epochs", required=False)
    parser.add_argument('--batch_size', type=int, default=1, help="batch size", required=False)
    parser.add_argument('--learning_rate', type=float, default=1e-4, help="learning rate", required=False)

    # 添加的参数
    parser.add_argument('--path', type=str, default=None, help="data path")
    parser.add_argument('--custom_chars', type=str, default=None, help="custom character set")
    parser.add_argument('--min_len', type=int, default=1, help="minimum SMILES length")
    parser.add_argument('--max_len', type=int, default=140, help="maximum SMILES length")
    parser.add_argument('--augmentation', type=int, default=10, help="data augmentation factor")
    parser.add_argument('--verbose', type=str, default='True', help="verbose mode")
    parser.add_argument('--block_size', type=int, default=140, help="model block size")
    parser.add_argument('--vocab_size', type=int, default=81, help="vocabulary size")
    parser.add_argument('--custom_tr', type=str, default=None, help="custom training set")
    parser.add_argument('--frac', type=float, default=0.9, help="training data fraction")

    args = parser.parse_args()  # py脚本用
    ...
    pre_model = f'{dir_path}/models/{args.pre_data}/{save_name}/'
    ft_model = f'{dir_path}/models/{args.data_name}/{save_name}/'
    ...
    if args.max_len:
        ...
        print("Using fixed max SMILES length: ", max_len)
    else:
        lens = [len(regex.findall(i.strip()))
                for i in tr_smiles + val_smiles]
        max_len = max(lens)
        print(f'Detected max SMILES length: {max_len}')

    # fix vocab for fine-tuning
    if args.custom_chars is not None:
        print("Using custom chars")
        whole_string = json.load(
            open(f'./datasets{args.path}/char/{args.custom_chars}_chars.json', 'r'))
        stoi = {ch: i for i, ch in enumerate(whole_string)}
        itos = {i: ch for i, ch in enumerate(whole_string)}
    
    else:
        raise ValueError('Please provide custom chars')

    print('Chars in vocab: ', itos)
    print('Padding token: ', itos[0], stoi['<'])

    # check if all tokens are in the vocabulary
    tr_smiles_filtered, tr_removed = filter_valid_smiles(tr_smiles, whole_string, regex)
    val_smiles_filtered, val_removed = filter_valid_smiles(val_smiles, whole_string, regex)

    tr_smiles = tr_smiles_filtered
    val_smiles = val_smiles_filtered
    ...
    model.load_state_dict(torch.load(f'{pre_model}{args.mpath}.pt'))
    ...
    tconf = TrainerConfig(max_epochs=args.max_epochs, batch_size=args.batch_size, learning_rate=args.learning_rate,
                          ...
                          num_workers=0, ckpt_path=f'{ft_model}{args.data_name}_aug{args.augmentation}_ft.pt')

    trainer = Trainer(model, train_dataset, valid_dataset, tconf)
    train_losses, val_losses = trainer.train()

    np.savetxt(
        f'{ft_model}{args.data_name}_{args.augmentation}_tr_losses.txt', train_losses)
    np.savetxt(
        f'{ft_model}{args.data_name}_{args.augmentation}_val_losses.txt', val_losses)
